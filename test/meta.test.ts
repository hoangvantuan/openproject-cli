...
describe("meta failure modes and surface", () => {
  ...
  test("project-scoped lookups render honestly until issue #5 fills them", async () => {
    const root = await makeTempRoom("op-cli-meta-future-");
    const { configDir, cacheDir } = await writeSingleProfile(root, "https://op.example");
    installMockApi(baseInstall("https://op.example"));
    const env = { OP_CLI_CONFIG_DIR: configDir, OP_CLI_CACHE_DIR: cacheDir };

    await run(["meta", "types"], env, {});
    const members = await run(["meta", "members"], env, {});
    const activities = await run(["meta", "activities"], env, {});

    expect(members).toEqual({ stdout: "ID  NAME\n", stderr: "", exitCode: 0 });
    expect(activities).toEqual({ stdout: "ID  NAME  DEFAULT\n", stderr: "", exitCode: 0 });
  });
});
